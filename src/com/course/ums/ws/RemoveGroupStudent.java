...
public class RemoveGroupStudent extends MyRoute {
    @Override
    public Object myHandle(Request request, Response response) throws Exception {
        ...
        if(resultSet.first()) {
            PreparedStatement preparedStatement = DBManager.getConnection().prepareStatement("DELETE FROM group_students where groups_id = (?) end students_id = (?) and semesters_id = (?) VALUES(?,?,?)", PreparedStatement.RETURN_GENERATED_KEYS);
